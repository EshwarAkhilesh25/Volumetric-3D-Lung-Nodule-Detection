...
import numpy as np
import torch
module_path = '/kaggle/input/lung-nodule-3/lung_nodule_copy2/lung_nodule_ct_detection/scripts'
...
def main():
    ...
    parser.add_argument(
        "-e",
        "--environment-file",
        default="/kaggle/working/environment_luna16_fold4.json",
        help="environment json file that stores environment path",
    )
    parser.add_argument(
        "-c",
        "--config-file",
        default="/kaggle/input/lung-nodule-3/lung_nodule_copy2/LUNA16_datasplit/config_train_luna16_16g.json",
...
#     args = parser.parse_args()
    ...
    if args.environment_file:
        print(f"Using environment file: {args.environment_file}")
    if args.config_file:
...
#     print(f"Load model from {env_dict['model_path']}")
    ...
    # 2) build network
    model_path = "/kaggle/input/fold4-160/model_luna16_fold4.pt"
    ...
    with torch.no_grad():
        ...
        for inference_data in inference_loader:
            ...
            for inference_img_filename, inference_pred_i in zip(inference_img_filenames, inference_data):
                boxes_2d = []
                for box in inference_pred_i["pred_box"].cpu().detach().numpy().tolist():
                    x_min = box[0] - box[3] / 2  # x_center - width / 2
                    y_min = box[1] - box[4] / 2  # y_center - height / 2
                    width = box[3]
                    height = box[4]
                    boxes_2d.append([x_min, y_min, width, height])

                result = {
                    "image_id": inference_img_filename,
                    "category_id": 1,
                    "box": boxes_2d,
